...
def plot_comparison(df, result_dist3, result_dist4, save_path=None):
    ...
    def draw_axis(ax, result, min_dist_label, color_theme):
        ax.set_title(
            f'MIN_DIST = {min_dist_label} (有效笔: {len(result["strokes"])})',
            fontsize=14,
            fontweight="bold",
            color=color_theme,
        )
        ...
        stroke_indices = [
            (idx - offset, t) for idx, t in strokes if offset <= idx < offset + num_bars
        ]
        ...
        replaced_indices = [
            (idx - offset, t) for idx, t in replaced if offset <= idx < offset + num_bars
        ]
    ...
    axes[1].set_xticklabels(
        [plot_df[col_dt].iloc[i].strftime("%m-%d") for i in range(0, len(plot_df), step)],
        rotation=45,
        fontsize=9,
    )
    ...
    legend_elements = [
        ...
        plt.Line2D(
            [0], [0], marker="v", color="w", markerfacecolor="red", markersize=10, label="顶分型(T)"
        ),
        plt.Line2D(
            [0],
            [0],
    ...
    stats_text = (
        f"MIN_DIST=3: {strokes3}笔, {replaced3}替换 | MIN_DIST=4: {strokes4}笔, {replaced4}替换"
    )
    fig.text(
        0.5,
        0.02,
...
def plot_diff_highlight(df, result_dist3, result_dist4, save_path=None):
    ...
    ax.set_title(
        f"笔识别差异对比 (红色圆圈 = MIN_DIST=4 跳过的分型)", fontsize=14, fontweight="bold"
    )
    ...
    stroke_indices3 = [
        (idx - offset, t) for idx, t in strokes3_only if offset <= idx < offset + num_bars
    ]
    stroke_indices4 = [
        (idx - offset, t) for idx, t in strokes4_only if offset <= idx < offset + num_bars
    ]
    ...
    for idx, f_type in stroke_indices3:
        ...
        if f_type == "TOP":
            price = plot_df[col_high].iloc[idx]
            ax.scatter(
                idx, price, marker="o", s=250, c="red", zorder=5, edgecolors="darkred", linewidths=2
            )
        else:
            price = plot_df[col_low].iloc[idx]
            ax.scatter(
    ...
    stroke_indices = [
        (idx - offset, t) for idx, t in all_strokes if offset <= idx < offset + num_bars
    ]
    stroke_sorted = sorted(stroke_indices, key=lambda x: x[0])
    ...
    legend_elements = [
        ...
        plt.Line2D(
            ...
            label=f"MIN_DIST=3独有分型 (跳过{len(strokes3_only)}个)",
        ),
        plt.Line2D(
            [0],
            [0],
