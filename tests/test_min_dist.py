...
def compare_min_dist_effects(input_path):
    ...
    strokes4_t = sum(1 for _, _, t in [(i, t, _) for i, t in strokes4] if t == 'TOP') if False else sum(1 for _, t in strokes4 if t == 'TOP')
    ...
    print("-" * 65)
    print(f"{'有效笔端点数量':<25} {len(strokes3):<15} {len(strokes4):<15} {len(strokes4)-len(strokes3):+d}")
    print(f"{'被替换分型数量':<25} {len(replaced3):<15} {len(replaced4):<15} {len(replaced4)-len(replaced3):+d}")
    print(f"{'顶分型(TOP)数量':<25} {strokes3_t:<15} {strokes4_t:<15} {strokes4_t-strokes3_t:+d}")
    print(f"{'底分型(BOTTOM)数量':<25} {strokes3_b:<15} {strokes4_b:<15} {strokes4_b-strokes3_b:+d}")
